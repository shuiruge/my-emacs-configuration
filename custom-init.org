...
#+begin_src emacs-lisp
  (defun find-config ()
    (interactive)
    (find-file (expand-file-name (concat "~/" ".emacs.d/" "custom-init.org"))))
#+end_src
...
#+begin_src emacs-lisp
    (defun reload-config()
      (interactive)
      (load-file (expand-file-name (concat "~/" ".emacs.d/" "init.el"))))
#+end_src
...
Because of a bug of capf, corfu cannot work. In some situations, it will keep inserting tabs. Reference: https://github.com/minad/corfu/issues/61.
...
Major mode for editing Agda files.
TODO
...
The basic rule of naming key chords is acronyms. For directions, use VIM keys.

#+begin_src emacs-lisp
  ;; buffer
  (key-chord-define-global "bl"     'switch-to-buffer)  ; short for buffer list.
  ...
  ;; move
  (key-chord-define-global "gw"     'forward-word)  ; short for go + VIM key.
  ...
  ;; scrolling
  (key-chord-define-global "sj"     'evil-scroll-page-down)  ; scroll down.
  (key-chord-define-global "sk"     'evil-scroll-page-up)    ; scroll up.
  ;; edit
